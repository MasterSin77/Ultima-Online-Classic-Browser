(function () {
  ...
  const chatEcho = false; // Set to true to output all logs via player.say()
  ...
  function* generateValidPaths(maxLength, path = [], pos = [0, 0], visited = new Set(["0,0"])) {
    if (pos[0] === 2 && pos[1] === 2) {
      yield path;
      return;
    }
    ...
    for (const dir of Object.values(directionButtons)) {
  ...
  function openGumpWithRemoveTrap() {
    journal.clear();
    const now = Date.now();
  ...
  function tryPath(path) {
    ...
    let gump = openGumpWithRemoveTrap();
    if (!gump || !gump.exists) {
      log("❌ Gump not available.");
      return false;
    }
    ...
    for (let i = 0; i < path.length; i++) {
      ...
      const failText = journal.waitForText("You fail to disarm the trap and reset it.", null, config.journalStepTimeoutMs);
    ...
    const result = journal.waitForTextAny([
      "You successfully disarm the trap!",
      "You fail to disarm the trap"
    ], null, config.finalResultTimeoutMs);
  ...
  function logSolvedPath(path) {
    ...
    player.say(`Remove Trap Solved: ${raw}`);
